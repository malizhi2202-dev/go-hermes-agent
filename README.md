...
- 普通输入直接对话
- `/new`、`/clear` 管理当前控制台工作流状态
- `/help` 查看控制台命令
- `/login` 在控制台里切换账号
- `/status`、`/usage`、`/insights` 看当前控制台与近期使用概况
- `/prompt-inspect`、`/prompt-cache-stats`、`/prompt-cache-clear`、`/prompt-config` 做 prompt 装配与缓存排障
- `/discover-models`、`/auxiliary-info`、`/auxiliary-chat`、`/auxiliary-switch` 也已经进控制台，模型发现和辅助链路不再只停留在顶层子命令
- `/extension-hooks`、`/extension-refresh`、`/extension-state`、`/extension-validate` 以及 `/multiagent-traces`、`/multiagent-summary`、`/multiagent-verifiers`、`/multiagent-failures`、`/multiagent-hotspots` 也已进控制台
- `/sessions`、`/history`、`/search`、`/audit` 做会话和排障
- `/resume` 现在会把一个已存 session 绑定为当前控制台活动会话，后续普通输入会继续写入该 session
- `/retry`、`/undo` 可以重跑或回滚当前控制台里新建的最近一轮独立 session
- `/models`、`/model`、`/model-metadata` 做模型查看、切换和元数据观测
- `/extensions`、`/tools` 做扩展和工具治理
- `/tool-exec`、`/execution-audit`、`/execution-profile-audit` 做安全工具与执行审计排障
- `/multiagent-plan`、`/multiagent-run`、`/multiagent-replay`、`/multiagent-resume` 做多 Agent 操作
...
- 审计与观测：`audit` / `execution-audit` / `execution-profile-audit` / `usage` / `insights`
